...
import { useProcessClauses } from "./hooks";
import type { ProcessClause, ProcessClauseSortField, SortDirection } from "./types";
import { ProcessClauseFormDialog } from "./ProcessClauseFormDialog";
import { DeleteProcessClauseDialog } from "./DeleteProcessClauseDialog";
...
function SortHeader({
  ...
  label: string;
  field: ProcessClauseSortField;
  activeField: ProcessClauseSortField;
  direction: SortDirection;
  onSort: (f: ProcessClauseSortField) => void;
}) {
...
export function ProcessClausesView() {
  const [searchInput, setSearchInput] = useState("");
  const search = useDebounced(searchInput, 300);
  const [sortField, setSortField] = useState<ProcessClauseSortField>("sort_order");
  const [sortDirection, setSortDirection] = useState<SortDirection>("asc");
  const [page, setPage] = useState(1);

  const [formOpen, setFormOpen] = useState(false);
  const [editing, setEditing] = useState<ProcessClause | null>(null);
  const [deleting, setDeleting] = useState<ProcessClause | null>(null);

  const query = useProcessClauses({
    search,
    sortField,
    sortDirection,
    page,
  ...
  function handleSort(field: ProcessClauseSortField) {
    if (field === sortField) {
  ...
  function openEdit(c: ProcessClause) {
    setEditing(c);
    setFormOpen(true);
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-foreground">Process Clauses</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Manage Process Clauses within each Framework Version.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-lg border border-border bg-card">
        <div className="flex flex-col gap-3 border-b border-border p-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:max-w-sm">
            ...
            <input
              ...
              placeholder="Search by framework, version, code, name, or description"
              aria-label="Search process clauses"
          ...
          <div className="border-b border-border bg-destructive/5 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-foreground">Couldn't load process clauses</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {friendlyProcessClauseError(query.error)}
                </p>
              </div>
              <button
                type="button"
        ...
        {query.isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
            ...
            <h3 className="text-base font-semibold text-foreground">No process clauses yet</h3>
            <p className="mt-1 max-w-sm text-sm text-muted-foreground">
              Create your first process clause for a Framework Version.
            </p>
            <button
              type="button"
              onClick={openCreate}
          ...
          <div className="px-6 py-12 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              No process clauses match "{search}".
            </p>
          </div>
        ) : (
            ...
            {rows.map((c) => {
              ...
              return (
                <li key={c.id} className="grid grid-cols-12 items-center gap-3 px-4 py-3 text-sm">
                  ...
                  <div className="col-span-3 hidden truncate text-xs text-muted-foreground md:block">
                    {c.description ?? "—"}
                  </div>
      ...
      <ProcessClauseFormDialog open={formOpen} onOpenChange={setFormOpen} clause={editing} />
      <DeleteProcessClauseDialog
