...
import { useDeleteRequirement, useRequirementDependencies } from "./hooks";
...
export function DeleteRequirementDialog({ open, onOpenChange, requirement }: Props) {
  const del = useDeleteRequirement();
  const deps = useRequirementDependencies(requirement?.id, open);
  ...
  const depList = counts
    ? [
        { label: "Assessment Criteria", singular: "Assessment Criterion", count: counts.assessment_criteria },
        {
          label: "Process Clause mappings",
          singular: "Process Clause mapping",
          count: counts.requirement_process_clauses,
        },
        { label: "Review Comments", singular: "Review Comment", count: counts.review_comments },
      ].filter((d) => d.count > 0)
    : [];
  ...
  async function handleDelete() {
    ...
    // or when any dependency count is greater than zero.
    if (loadingDeps || depError || !counts) return;
    const total =
      counts.assessment_criteria + counts.requirement_process_clauses + counts.review_comments;
    if (total > 0) return;
    try {
      await del.mutateAsync(requirement.id);
      toast.success("Requirement deleted");
      onOpenChange(false);
    } catch (err) {
      setError(friendlyRequirementError(err));
    }
  }

  const p = requirement.principle;
  const lv = p?.level;
  const fv = lv?.framework_version;
  const label = `${fv?.framework?.code ?? "—"} — Version ${fv?.version_number ?? ""} — ${lv?.code ?? ""} — ${p?.code ?? ""} · ${requirement.code} ${requirement.title}`;
      ...
      >
        <div className="flex items-start justify-between border-b border-border px-5 py-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 id="del-req-title" className="text-base font-semibold text-foreground">
                {hasDeps ? "Cannot delete Requirement" : "Delete requirement"}
              </h2>
              <div className="mt-1 text-sm text-muted-foreground">
                {loadingDeps ? (
                  ...
                  <p>
                    Couldn't verify dependent records ({friendlyRequirementError(deps.error)}). Retry
                  ...
                  <>
                    ...
                    <ul className="mt-2 list-disc space-y-0.5 pl-5">
                      {depList.map((d) => (
                        <li key={d.label}>
                          {d.count} {d.count === 1 ? d.singular : d.label}
                        </li>
                      ))}
                    </ul>
                    <p className="mt-2">Remove or reassign these records first.</p>
                  </>
                ) : (
                  <p>
